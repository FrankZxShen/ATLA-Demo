...
from pythia.common.registry import registry
from pythia.utils.twa_utils import create_batch_labels, create_batch_labels_diag
...
class CaptionCrossEntropyLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        if hasattr(sample_list, "caption_len"):
            caption_lengths, _ = sample_list.caption_len.sort(
...
def loss_jocor_kl(pred, soft_targets, reduce=True):
...
def loss_jocor_kl2(pred, soft_targets, reduce=True):
    kl = F.kl_div(F.log_softmax(pred, dim=0),F.softmax(soft_targets, dim=0),reduce=False)

    if reduce:
        return torch.mean(torch.sum(kl, dim=0))
    else:
        return torch.sum(kl, 0)
...
class M4CDecodingBCEWithMaskLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        epsilon = 1e-8  # small epsilon value to avoid zero
        # losses = torch.clamp(losses, 1e-4, 1e+4)
        losses *= loss_mask.unsqueeze(-1)
        count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
        loss = torch.sum(losses) / count
        # =====TARGET8: 计算KL Loss======
        if sample_list['adv'] == True:
            answer_prob = F.softmax(scores, dim=1)
            answer_logprob = F.log_softmax(scores, dim=1)
            answer_logprob = torch.clamp(answer_logprob, -1e+8 + epsilon, 0.0 - epsilon)
            sample_list['gt_answer_prob'] = torch.clamp(sample_list['gt_answer_prob'], epsilon, 1.0 - epsilon)
            answer_prob = torch.clamp(answer_prob, epsilon, 1.0 - epsilon)
            sample_list['gt_answer_logprob'] = torch.clamp(sample_list['gt_answer_logprob'], -1e+8 + epsilon, 0.0 - epsilon)
            kl_losses = F.kl_div(answer_logprob, sample_list['gt_answer_prob'], reduce=False) + \
                F.kl_div(sample_list['gt_answer_logprob'],
                         answer_prob, reduce=False)
            kl_losses *= loss_mask.unsqueeze(-1)
            kl_count = torch.max(torch.sum(loss_mask),
                                 self.one.to(kl_losses.device))
            kl_losses = torch.sum(kl_losses) / kl_count
            adv_kl_weight = 1.5  # 后面定义到yml中
            loss = loss + adv_kl_weight * kl_losses
    ...
    def forward(self, sample_list, model_output):
        ...
        if "contrastive_scores" in model_output:
            ...
            if "cons_labels" in sample_list:
                ...
                labels = torch.arange(0, logits_per_image.size(0)).to(
        ...
        if sample_list['adv'] == True:
            ...
            answer_prob = torch.clamp(answer_prob, epsilon, 1.0 - epsilon)
            sample_list['gt_answer_logprob'] = torch.clamp(sample_list['gt_answer_logprob'], -1e+8 + epsilon, 0.0 - epsilon)
            kl_losses = F.kl_div(answer_logprob, sample_list['gt_answer_prob'], reduction='none') + \
                F.kl_div(sample_list['gt_answer_logprob'],
                         answer_prob, reduction='none')
            kl_losses *= loss_mask.unsqueeze(-1)
            kl_count = torch.max(torch.sum(loss_mask),
                                 self.one.to(kl_losses.device))
            kl_losses = torch.sum(kl_losses) / kl_count
...
class pretrainonlyM4CDecodingBCEWithMaskLoss(nn.Module):
    def __init__(self):
        ...
        self.fuse_obj_weight = torch.nn.Parameter(
        ...
        self.fuse_vis_weight = torch.nn.Parameter(
            torch.FloatTensor(1), requires_grad=True)
    ...
    def forward(self, sample_list, model_output):
        ...
        if 'textcls_scores' in model_output:
            ...
            if "term_scores" in model_output:
                ...
                char_loss_mask = (
                    sample_list['adv_input_labels'] != -1).float()
                term_vocab_size = 30001  # 30001
                adv_term_losses = F.cross_entropy(model_output["term_scores"].view(
                    -1, term_vocab_size), sample_list['adv_input_labels'].view(-1), reduction="none", ignore_index=-1)
                ori_loss = self.pretrainloss(sample_list, model_output)
                adv_term_losses *= char_loss_mask.view(-1)
                count = torch.max(torch.sum(char_loss_mask),
                                  self.one.to(adv_term_losses.device))
                ...
                if "o2r_labels" in sample_list:
                    ...
                    if "ocr_contrastive_scores" in model_output:
                        ...
                        loss_i = loss_i + loss_i_2 * \
                            self.fuse_ocr_weight  # .to(loss_i_2.device)
                        loss_t = loss_t + loss_t_2 * \
                            self.fuse_ocr_weight  # .to(loss_i_2.device)
                    if "obj_contrastive_scores" in model_output:
                        ...
                        loss_i = loss_i + loss_obj * \
                            self.fuse_obj_weight.to(loss_obj.device)
                        loss_t = loss_t + loss_obj * \
                            self.fuse_obj_weight.to(loss_obj.device)
                        # loss_i = loss_obj
                        # loss_t = loss_obj
                    # and "obj_contrastive_scores" not in model_output:
                    if "vis_contrastive_scores" in model_output:
                        # print(sample_list.keys())
                        vis_labels = create_batch_labels_diag(
                            sample_list["vis_labels"])
                        logits_per_image_2 = model_output['vis_contrastive_scores']
                        logits_per_text_2 = logits_per_image_2.t()
                        mask = vis_labels != -1
                        ...
                        loss_i = loss_i + loss_i_vis * \
                            self.fuse_vis_weight.to(loss_i_vis.device)
                        loss_t = loss_t + loss_t_vis * \
                            self.fuse_vis_weight.to(loss_i_vis.device)
                ...
                if "deep_predict_scores" in model_output:
                    ...
                    deep_predict_scores = model_output['deep_predict_scores'].view(
                        -1)
                ...
                loss = loss * 0.0 + \
                    self.pretrainloss(sample_list, model_output)
            ...
            if "recover_scores" in model_output:
                ...
                count = torch.max(torch.sum(char_loss_mask),
                                  self.one.to(recover_losses.device))
...
class M4CDecodingPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # 1 is polluted, not compute MLM
        pollute_mask = (1-pollute_mask).repeat(1, loss_mask.shape[-1])
        loss_mask *= pollute_mask
        assert scores.dim() == 3 and loss_mask.dim() == 2
        # print("score:",scores) [x,35022,170]
        # print("targets",targets)[x,170]
        ...
        if sample_list['adv'] == True:
            ...
            epsilon = 1e-8  # small epsilon value to avoid zero
            answer_logprob = torch.clamp(answer_logprob, -1e+8 + epsilon, 0.0 - epsilon)
            sample_list['gt_textcls_prob'] = torch.clamp(sample_list['gt_textcls_prob'], epsilon, 1.0 - epsilon)
            answer_prob = torch.clamp(answer_prob, epsilon, 1.0 - epsilon)
            sample_list['gt_textcls_logprob'] = torch.clamp(sample_list['gt_textcls_logprob'], -1e+8 + epsilon, 0.0 - epsilon)
            kl_losses = F.kl_div(answer_logprob, sample_list['gt_textcls_prob'], reduction='none') + \
                F.kl_div(sample_list['gt_textcls_logprob'], answer_prob, reduction='none')
            kl_losses = kl_losses.mean(dim=1)
            # mean_value = torch.mean(kl_losses, dim=1, keepdim=True)  # 平均掉第二维这样做是有问题的
            # kl_losses = torch.squeeze(mean_value, dim=1)  # 压缩第二位维度
            # kl_losses *= loss_mask
            # kl_count = torch.max(torch.sum(loss_mask),
            #                      self.one.to(kl_losses.device))
            # kl_losses = torch.sum(kl_losses) / kl_count
...
class M4CDecodingPollutePretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # =====TARGET8: 计算KL Loss======
        # if sample_list['adv'] == True:
        #     pollutecls_prob = F.softmax(scores, dim=0)
        #     pollutecls_logprob = F.log_softmax(scores, dim=0)
        #     # print("pollutecls_prob: ",pollutecls_prob)
        #     # print("pollutecls_logprob: ",pollutecls_logprob)
        #     epsilon = 1e-8  # small epsilon value to avoid zero
        #     pollutecls_logprob = torch.clamp(pollutecls_logprob, -1e+8 + epsilon, 0.0 - epsilon)
        #     sample_list['gt_pollutecls_prob'] = torch.clamp(sample_list['gt_pollutecls_prob'], epsilon, 1.0 - epsilon)
        #     pollutecls_prob = torch.clamp(pollutecls_prob, epsilon, 1.0 - epsilon)
        #     sample_list['gt_pollutecls_logprob'] = torch.clamp(sample_list['gt_pollutecls_logprob'], -1e+8 + epsilon, 0.0 - epsilon)
        #     kl_loss = F.kl_div(pollutecls_logprob, sample_list['gt_pollutecls_prob'], reduce=False) + \
        #         F.kl_div(sample_list['gt_pollutecls_logprob'],
        #                  pollutecls_prob, reduce=False)
        #     # kl_loss = kl_loss.mean() * targets.size(0)

        #     # print("kl_loss_poll_1: ",kl_loss)
        #     adv_kl_weight = 1.5  # 后面定义到yml中
        #     loss = loss + (adv_kl_weight * kl_loss)
        #     # print("pollutecls_loss: ",loss)
        return loss
...
class M4CDecodingOverkapPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # =====TARGET8: 计算KL Loss======
        # if sample_list['adv'] == True:
        #     overlapcls_prob = F.softmax(scores.unsqueeze(-1), dim=0)
        #     overlapcls_logprob = F.log_softmax(scores.unsqueeze(-1), dim=0)
        #     epsilon = 1e-8  # small epsilon value to avoid zero
        #     overlapcls_logprob = torch.clamp(overlapcls_logprob, -1e+8 + epsilon, 0.0 - epsilon)
        #     sample_list['gt_overlapcls_prob'] = torch.clamp(sample_list['gt_overlapcls_prob'], epsilon, 1.0 - epsilon)
        #     overlapcls_prob = torch.clamp(overlapcls_prob, epsilon, 1.0 - epsilon)
        #     sample_list['gt_overlapcls_logprob'] = torch.clamp(sample_list['gt_overlapcls_logprob'], -1e+8 + epsilon, 0.0 - epsilon)
        #     kl_loss = F.kl_div(overlapcls_logprob, sample_list['gt_overlapcls_prob'], reduce=False) + \
        #         F.kl_div(sample_list['gt_overlapcls_logprob'],
        #                  overlapcls_prob, reduce=False)
        #     # kl_loss = kl_loss.mean() * targets.size(0)
