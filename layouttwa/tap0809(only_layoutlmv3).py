...
#         self.x_position_embeddings = nn.Embedding(1024, 128)
#         self.y_position_embeddings = nn.Embedding(1024, 128)
#         self.h_position_embeddings = nn.Embedding(1024, 128)
#         self.w_position_embeddings = nn.Embedding(1024, 128)
...
#         return embeddings
class Layoutlmv3make_position_ids(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.has_relative_attention_bias = config.has_relative_attention_bias
        self.has_spatial_attention_bias = config.has_spatial_attention_bias
        self.embeddings = LayoutLMv3TextEmbeddings(config)
    def forward(self, bbox, input_ids):
        input_shape = input_ids.size()
        if self.has_relative_attention_bias or self.has_spatial_attention_bias:
            ...
            final_bbox = bbox
            final_position_ids = None
        return final_bbox, final_position_ids
...
class M4C(BaseModel):
    def __init__(self, config):
        ...
        self.mmt_config = LayoutLMv3Config(**self.config.mmt)
    ...
    def _build_txt_encoding(self):
        ...
        # self.text_bert_config = RobertaConfig(**self.config.text_bert)
        self.text_bert_config = LayoutLMv3Config(**self.config.text_bert)
        if self.config.text_bert_init_from_bert_base:
            self.text_bert = TextBert.from_pretrained(
                'microsoft/layoutlmv3-base', config=self.text_bert_config
            )
            # self.text_bert = TextBert.from_pretrained(
            #     'roberta-base', config=self.text_bert_config
            # )
    ...
    def _build_ocr_encoding(self):
        ...
        # OCR Layoutlmv3 feature
        # self.ocr_layoutlmv3_config = LayoutLMv3Config(**self.config.ocr_layoutlmv3)
        # self.processor = AutoProcessor.from_pretrained("microsoft/layoutlmv3-base", apply_ocr=False)
        # self.layoutlmv3 = AutoModel.from_pretrained("microsoft/layoutlmv3-base", config=self.ocr_layoutlmv3_config)
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        if 'bert' in self.config.ocr.features:
            # print("sample_list.word_ids:",sample_list.word_ids)
            # print("shape:",sample_list.word_ids.shape)# [x,200]
            # print("ocr_bbox:",ocr_bbox)
            ocr_rem_id = sample_list.word_ids
            # print(sample_list.word_ids.shape)#[16,300]
            ocr_rem_mask = _get_mask_medpad(ocr_rem_id)
            ocr_rem_type_mask = _get_type_mask_medpad_ocr(ocr_rem_id)
            # print("ocr_shape:",ocr_bbox.shape)
            # print("ocr:",ocr_bbox*1000)
            # print("origin_shape:",ocr_origin.shape)#[x,200,4]
            # print("origin:",ocr_origin)
            if sample_list.adv == True:
       ...
       # print(char_sequence_repr.size())
        ...
        if self.fb_featrue != '':
            ...
            if 'bert' in self.fb_featrue and self.char_vocab_decoder:
                ...
                vocab_bert_out = self.text_bert_out_linear(vocab_bert_out)
...
class TextBert(LayoutLMv3PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.embeddings = LayoutLMv3TextEmbeddings(config)
        self.encoder = RobertaEncoder(config)
        self.make_positions_ids = Layoutlmv3make_position_ids(config)
        self.init_weights()  

    def forward(self, txt_inds, txt_mask, adv=False, txt_type_mask=None, bbox=None, adv_delta_txt=None, adv_delta_ocr=None):
        ...
        if adv == True:
            if adv_delta_txt != None:
                ...
                encoder_inputs = encoder_inputs + adv_delta_ocr
        attention_mask = txt_mask
...
class MMT(LayoutLMv3PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
def _get_type_mask_medpad(token_id):
    ...
    type_mask = torch.zeros(token_id.shape).cuda() * 1.
    type_mask[:, :20] = 0
