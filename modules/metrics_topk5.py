...
class TextVQAAccuracy(BaseMetric):
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        _, pred_candidates = torch.topk(model_output["scores"],5,dim=-1)
        ...
        for idx in range(batch_size):
            ...
            for cad_id in range(5):
                answer_words_cand = [[]] * 5 
                for answer_id in pred_candidates[:,:,cad_id][idx].tolist():
...
@registry.register_metric("caption_test")
class CaptionForce(TextVQAAccuracy):
    def __init__(self):
        super().__init__()
        self.name = "textcaps_bleu4"
        self.gt_key = 'ref_strs'
