...
from pythia.utils.twa_utils import create_obj_labels, findRelatedOCR_adr, findRelatedOCR_ori,  object_bert_ids, add_ocr_char_info, create_ocr_char_info,findRelatedOCR, add_cons_ocr_info,create_same_labels
...
class M4CTextVQADataset(TextVQADataset):
    ...
    def load_item(self, idx):
        #idx = 1000+idx
        image_path = self.imdb[idx]['image_path']
        ...
        current_sample = self.add_sample_details(sample_info, current_sample, image_path)
    ...
    def normalize_bbox(self, origin, width, height):
        # return [
        #     1000 * (bbox[0] / width),
        #     1000 * (bbox[1] / height),
        #     1000 * (bbox[2] / width),
        ...
        return origin
        # return [x0,y0,x1,y1]
    def normalize_obj_bbox(self, origin, width, height, w, h):
        for bbox in origin:
            bbox[0] = int(w * (bbox[0] / width))
            bbox[1] = int(h * (bbox[1] / height))
            bbox[2] = int(w * (bbox[2] / width))
            bbox[3] = int(h * (bbox[3] / height))
        return origin
        # return [x0,y0,x1,y1]

    def add_sample_details(self, sample_info, sample, image_path):
        ...
        img_path1_stvqa = 'data/' + file1 + 'train/' + image_path
        img_path1_stvqa_test = 'data/' + file1 + 'test/' + image_path
        img_path2 = 'data/' + file2 + 'train/' + image_path
        img_path2_test = 'data/' + file2 + 'test/' + image_path
        ...
        if os.path.exists(img_path1):
        ...
        elif os.path.exists(img_path1_stvqa):
            img = Image.open(img_path1_stvqa).convert("RGB")
            sample.img = img
            # width_old, height_old = img.size
            target_size = (500,384)
            img = img.resize(target_size)
            # width, height = img.size
            sample.image = transforms.ToTensor()(img)
        elif os.path.exists(img_path1_stvqa_test):
            img = Image.open(img_path1_stvqa_test).convert("RGB")
            sample.img = img
            # width_old, height_old = img.size
            target_size = (500,384)
            img = img.resize(target_size)
            ...
            # width, height = img.size
            sample.image = transforms.ToTensor()(img)
        elif os.path.exists(img_path2_test):
            img = Image.open(img_path2_test).convert("RGB")
            sample.img = img
            # width_old, height_old = img.size
            target_size = (500,384)
            img = img.resize(target_size)
            ...
            print(img_path1_stvqa)
            print("No images!")
            # print(img_name)
            # sample.img = torch.zeros(3,500,384)
            sample.image = torch.zeros(3,384,500)
        # print("img.shape:",sample.image.shape)#[3,384,500]
        # print("crnnshape:",sample.image_feature_0.shape)#[100,2048]
        ...
        # obj_bbox_origin = self.normalize_obj_bbox(sample['image_info_0']['bbox'], sample_info["image_width"], sample_info["image_height"], width, height)
        ...
        # sample.obj_bbox_origin_coordinates = self.copy_processor(
        #     {"blob": np.array(obj_bbox_origin, dtype=np.int64)}
        # )["blob"][:obj_max_len]
        ## Load OCR bert token
        ...
        if self.msocr:   ## load ms ocr prediction directly from _info.npy; else: from imdb provided
            sample.ocr_tokens = sample.image_info_1["ocr_tokens"]
        ...
        sample.context_tokens_enc = enc_obj2bytes(context["tokens"],max_size=8188)
        ...
        if self.msocr:
            ...
            ocr_origin = np.zeros((0,4),dtype=np.float32)
            if sample.image_info_1['ocr_boxes'].shape[0]!=0:
                ...
                # ocr_origin = 1000 * (sample.image_info_1['ocr_boxes'] / [sample_info["image_width"], sample_info["image_height"], sample_info["image_width"], sample_info["image_height"]])
                # for onebox,w,h in zip(sample.image_info_1['ocr_boxes'],sample_info["image_width"],sample_info["image_height"]):
                ocr_origin = self.normalize_bbox(sample.image_info_1['ocr_boxes'],sample_info["image_width"],sample_info["image_height"])
            # if np.all(ocr_origin) > 1023:
            #     for index in ocr_origin:
            #         for i in index:
            #             if i > 1023:
            #                 print("出错位置：",i)
            #                 print("此时的wh：",sample_info["image_width"]," ",sample_info["image_height"])
            # assert np.all(ocr_origin) <= 1023
            sample.ocr_bbox_coordinates = self.copy_processor(
                {"blob": np.array(ocr_bbox, dtype=np.float32)}
            )["blob"][:max_len]
            sample.ocr_bbox_origin = self.copy_processor(
                {"blob": np.array(ocr_origin, dtype=np.int64)}
            )["blob"][:max_len]# [100,4]
            # for origin in sample.ocr_bbox_origin:
            #     if len(origin) == 1:
            #         continue
            #     origin = self.normalize_bbox(origin, sample_info["image_width"], sample_info["image_height"])
        else:
            if 'ocr_normalized_boxes' in sample_info:
    ...
    def add_answer_info(self, sample_info, sample):
        ...
        if sample_has_answer:
            ...
            sample.gt_answers_enc = enc_obj2bytes(answers,max_size=8188)
