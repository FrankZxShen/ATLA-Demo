...
class M4CTextVQADataset(TextVQADataset):
    def __init__(self, dataset_type, imdb_file_index, config, *args, **kwargs):
        ...
        self.object_clsname = [x.strip() for x in list(
            open('data/1600-400-20/objects_vocab.txt', 'r'))]
        ...
        if self.msocr:
            assert('fc6' not in config.image_features.train[0])
        ...
        if self.pretrain and self._dataset_type == 'train' and self.textcap_pretrain:
            ...
            capjson = json.load(
                open('data/original_dl/TextCaps_0.1_train.json', 'r'))
            for ii in range(len(capjson['data'])):
                imid, cap, refcap = capjson['data'][ii]['image_id'], capjson[
                    'data'][ii]['caption_str'], capjson['data'][ii]['reference_strs']
        ...
        if self._use_features is True:
            ...
            if 'object_tokens' not in features['image_info_0']:
                features['image_info_0']['object_tokens'] = \
                    [self.object_clsname[x]
                        for x in features['image_info_0']['objects']]
    ...
    def add_sample_details(self, sample_info, sample):
        ...
        if self.pretrain and self._dataset_type == 'train' and random.random() < 0.5 and self.textcap_pretrain:
            if sample_info['image_id'] in self.textcap:  # exclude stvqa samples
                question_str = self.textcap[sample_info['image_id']][random.randint(
                    0, len(self.textcap[sample_info['image_id']])-1)]
        # end of caption data pretrain

        processed_question = self.text_processor(
            {"question": question_str}, updatelen=20)
        if self.pretrain:
            processed_question, output_label = self.random_word(
                processed_question, self.text_processor.bert_tokenizer.vocab)
            sample.text_mask_label = output_label
        else:
            sample.text_mask_label = None
        sample.text = processed_question['token_inds']
        # text_len include CLS, SEP
        sample.text_len = processed_question['token_num']
        #######################################################################
        # 2. Load object
        # object bounding box information
        obj_bbox = sample['image_info_0']['bbox'] * [1./sample_info["image_width"], 1. /
                                                     sample_info["image_height"], 1./sample_info["image_width"], 1./sample_info["image_height"]]
        ...
        processed_obj = self.text_processor(
            {"question": obj_str}, updatelen=obj_str_len)
        if self.pretrain:
            processed_obj, output_label = self.random_word(
                processed_obj, self.text_processor.bert_tokenizer.vocab)  # , mask_prob=0.)
        ...
        if self.msocr:  # load ms ocr prediction directly from _info.npy; else: from imdb provided
            ...
            if 'ocr_conf' not in sample.image_info_1:
                sample.image_info_1['ocr_conf'] = None
        ...
        processed_ocr = self.text_processor(
            {"question": ocr_str}, updatelen=ocr_str_len)

        # processed_ocr = self.text_processor({"question": ocr_str}, updatelen=20)
        if self.pretrain:
            processed_ocr, output_label = self.random_word(
                processed_ocr, self.text_processor.bert_tokenizer.vocab)
            ...
            sample.tag_pollute = (
                sample.ocrtag_pollute + sample.langtag_pollute + sample.objtag_pollute).clamp(max=1)
        else:
        ...
        if self.msocr:
            ...
            if sample.image_info_1['ocr_boxes'].shape[0] != 0:
                ocr_bbox = sample.image_info_1['ocr_boxes'] * [1./sample_info["image_width"], 1. /
                                                               sample_info["image_height"], 1./sample_info["image_width"], 1./sample_info["image_height"]]
        ...
        sample.overlap = self.region_overlap(
            sample.obj_bbox_coordinates, sample.ocr_bbox_coordinates[:, :4], threshold=0.99)
        targetoverlap = torch.tensor(
            0) if random.random() < 0.5 else torch.tensor(1)
        ...
        if len_index != 0:
            ...
            sample.overlap_obj, sample.overlap_ocr = torch.tensor(
                0), torch.tensor(0)
        return sample
    ...
    def region_overlap(self, bbox_obj, bbox_ocr, threshold=0.99, bgimgsize_thes=0.25, bgobj_mask=None, eps=1e-6):
        ...
        area_obj = (bbox_obj[:, 2] - bbox_obj[:, 0]) * \
            (bbox_obj[:, 3] - bbox_obj[:, 1])
        area_ocr = (bbox_ocr[:, 2] - bbox_ocr[:, 0]) * \
            (bbox_ocr[:, 3] - bbox_ocr[:, 1])
        ...
        cover = area_obj.unsqueeze(1).repeat(
            1, area_ocr.shape[0]) / (area_obj[:, None] + area_ocr - inter)
        ...
        for i, token in enumerate(token_inds):
            ...
            if prob < mask_prob and token not in [0, 101, 102]:
                ...
                # 10% randomly change token to random token
                    ...
                    token_inds[i] = random.choice(
                        list(range(1000, len(tokenizer))))  # [0]
    ...
    def random_word_answer(self, processed_answers, mask_prob=0.15):
        ...
        for ii in range(pretrain_prev_inds.shape[0]):
            ...
            if prob < mask_prob and pretrain_prev_inds[ii] != 0 and pretrain_prev_inds[ii] != 1:
                ...
                pretrain_targets[ii,
                                 :] = processed_answers["answers_scores"][ii-1, :]
                prob /= mask_prob
                if prob < 0.8:
                    pretrain_prev_inds[ii] = 3  # <unk>
                elif prob < 0.9:
                    pretrain_prev_inds[ii] = random.choice(
                        list(range(4, pretrain_targets.shape[1]-1)))
                else:
