...
#         count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
#         loss = torch.sum(losses) / count
#         if True and 'textcls_scores' in model_output:
...
class M4CDecodingPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # =====TARGET8: 计算KL Loss======
        if sample_list['adv'] == True:
            textcls_prob = F.softmax(scores, dim=1)
            textcls_logprob = F.log_softmax(scores, dim=1)
            # print("textcls_prob: ",textcls_prob)
            # print("textcls_logprob: ",textcls_logprob)
            kl_losses = F.kl_div(textcls_logprob, sample_list['gt_textcls_prob'], reduction='none') + \
                F.kl_div(sample_list['gt_textcls_logprob'],
                         textcls_prob, reduction='none')
            # kl_loss = kl_loss.mean() * targets.size(1)
            mean_value = torch.mean(kl_losses, dim=1, keepdim=True)  # 平均掉第二维
            kl_losses = torch.squeeze(mean_value, dim=1)  # 压缩第二位维度
            kl_losses *= loss_mask
            kl_count = torch.max(torch.sum(loss_mask),
                                 self.one.to(kl_losses.device))
            kl_loss = torch.sum(kl_losses) / kl_count
            # print("kl_loss_text_1: ",kl_loss)
            adv_kl_weight = 1.5  # 后面定义到yml中
...
class M4CDecodingPollutePretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        if sample_list['adv'] == True:
            ...
            kl_loss = F.kl_div(pollutecls_logprob, sample_list['gt_pollutecls_prob'], reduction='mean') + \
                F.kl_div(sample_list['gt_pollutecls_logprob'],
                         pollutecls_prob, reduction='mean')
...
class M4CDecodingOverkapPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # =====TARGET8: 计算KL Loss 可能出现问题======
        # if sample_list['adv'] == True:
        #     overlapcls_prob = F.softmax(scores[mask], dim=0)
        #     overlapcls_logprob = F.log_softmax(scores[mask], dim=0)
        #     # print("overlapclscls_prob: ",overlapcls_prob)
        #     # print("overlapclscls_logprob: ",overlapcls_logprob)
        #     kl_loss = F.kl_div(overlapcls_logprob, sample_list['gt_overlapcls_prob'], reduction='mean') + \
        #                 F.kl_div(sample_list['gt_overlapcls_logprob'], overlapcls_prob, reduction='mean')
        #     kl_loss = kl_loss.mean() * targets[mask].size(0)
        #     adv_kl_weight = 1.5 # 后面定义到yml中
        #     loss = (loss + adv_kl_weight * kl_loss) / sample_list['adv_step']
        #     # print("overlapclsloss: ",loss)
        # ## if cls
        # losses = F.cross_entropy(
