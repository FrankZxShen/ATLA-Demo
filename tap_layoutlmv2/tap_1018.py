...
class M4C(BaseModel):
    ...
    def _build_obj_encoding(self):
        ...
        self.linear_obj_feat_to_mmt_in = nn.Linear(
            1536, self.mmt_config.hidden_size
        )
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        obj_bbox = sample_list.obj_bbox_coordinates #[x,100,4]
        ...
        if obj_feat.size(1) < 200:
            obj_feat_expand = torch.zeros(obj_feat.size(0),200-obj_feat.size(1),obj_feat.size(2)).to(obj_feat.device)
            obj_feat = torch.cat([obj_feat,obj_feat_expand],dim=1)
        obj_feat = obj_feat.view(obj_feat.size(0),sample_list.obj_bbox_coordinates.size(1),1536)
