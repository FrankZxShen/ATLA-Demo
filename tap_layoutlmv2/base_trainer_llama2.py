...
class BaseTrainer:
    ...
    def train(self):
        ...
        if self.config.training_parameters.test:
    ...
    def _forward_pass(self, batch):
        prepared_batch = self.dataset_loader.prepare_batch(batch)
        self.profile("Batch prepare time")
        # print(prepared_batch.keys())
        # print(prepared_batch)
        # print(prepared_batch['question_str'])
        # Arguments should be a dict at this point
        if self.use_llama2:
        ...
        # print(report.metrics)#{'train/m4c_textvqa/textvqa_accuracy': tensor([0.], device='cuda:0')}
        self.profile("Forward time")
    ...
    def evaluate(self, loader, use_tqdm=False, single_batch=False):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(loader, disable=disable_tqdm):
                ...
                prepared_batch['adv'] = False
                # print(prepared_batch.keys())
                # print(prepared_batch['gt_answers_enc'])
                # print(prepared_batch) 
                model_output = self.model(prepared_batch)
                # print(model_output)
