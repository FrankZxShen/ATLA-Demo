...
from pythia.utils.modeling_llama import *
...
from transformers import (
    ...
    BlipForConditionalGeneration,Blip2ForConditionalGeneration, Blip2Processor,
    LlamaTokenizer,
    )
from pythia.utils.activations_utils import ACT2FN
...
class M4C(BaseModel):
    ...
    def _build_mmt(self):
        # self.llm = LlamaForCausalLM.from_pretrained('/home/szx/project/TAP/data/models/llama-2-7b-hf',config=self.mmt_config)
    ...
    def _build_vlm(self):
        # blip
        # model_blip = BlipForConditionalGeneration.from_pretrained('/DATACENTER1/szx/tap/data/models/blip-caption-large/', torch_dtype=torch.float32)
        # self.caption_processor = AutoProcessor.from_pretrained('/DATACENTER1/szx/tap/data/models/blip-caption-large/')
        # self.model_blip = model_blip

        # blip2-opt-2.7b
        model_blip = Blip2ForConditionalGeneration.from_pretrained('/DATACENTER1/szx/tap/data/models/blip2-opt-2.7b-coco/',load_in_8bit=True,device_map={"":0},torch_dtype=torch.float16)
        self.caption_processor = Blip2Processor.from_pretrained('/DATACENTER1/szx/tap/data/models/blip2-opt-2.7b-coco/')
        self.model_blip = model_blip
        # self.stvqa2vlm = nn.linear()
        for name, para in self.model_blip.named_parameters():
            # 权重全部冻结
            para.requires_grad_(False)
    
    def _build_llm(self):
        self.llm_tokenizer = LlamaTokenizer.from_pretrained('/home/szx/project/TAP/data/models/llama-2-7b-hf', use_fast=False, truncation_side="left")
        self.llm_model = LlamaForCausalLM.from_pretrained(
            '/home/szx/project/TAP/data/models/llama-2-7b-hf', torch_dtype=torch.float16
        )
        self.llm_out = LLM_out(self.llm_tokenizer,self.llm_model)
    ...
    def forward(self, sample_list):
        ...
        results["llm_loss"] = fwd_results["llm_loss"]
        # self._forward_projection_vlm(sample_list, fwd_results)
        if self.pretrain:
    ...
    def _forward_projection_vlm(self, sample_list, fwd_results): 
        print('======================')    
        print("1:",fwd_results["scores"].shape)
        prompt = "Answer: "
        inputs = self.caption_processor(images=sample_list.image, text=prompt, return_tensors="pt").to(sample_list.image_feature_0.device)
        print("2:",inputs)
        tokens = self.model_blip.generate(**inputs, max_new_tokens=64)
        self.caption_processor.batch_decode(tokens, skip_special_tokens=True)[0].strip()
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        if ocr_fc6.size(1) < ocr_len:
            zeros_to_insert = torch.zeros((ocr_fc6.size(0),ocr_len-ocr_fc6.size(1),2048)).to(ocr_fc6.device)
            ocr_fc6 = torch.cat([ocr_fc6,zeros_to_insert],dim=1)
        if sample_list.adv == True:
            ocr_fc6 = ocr_fc6 + sample_list.adv_delta_ocr_img
        ocr_fc7 = self.ocr_faster_rcnn_fc7(ocr_fc6)
        if self.con_mnt_rel:
            ocr_fc7 = torch.cat([ocr_fc7, ocr_fc7], dim=-2)
        ocr_fc7 = F.normalize(ocr_fc7, dim=-1)# [x,300,2048]
    ...
    def _forward_llm(self, sample_list, fwd_results):
        
        outputs = self.llm_out(sample_list, fwd_results)
        fwd_results['llm_loss'] = outputs.loss
        print(outputs.loss)
    ...
    def _forward_mmt_and_output(self, sample_list, fwd_results):
        if self.training:
            ...
            self._forward_llm(sample_list, fwd_results)
...
class MMT(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class LLM_out(nn.Module):
    ...
    def concat_text_input(self, input_ids, input_atts, output_ids, output_atts):
    ...
    def forward(
        ...
        inputs_llm = self.llm_proj(fwd_results['mmt_seq_output'])
        atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(fwd_results['mmt_seq_output'].device)

        self.llm_tokenizer.padding_side = "right"
        self.llm_tokenizer.truncation_side = 'left'
        # print(samples['question_str'])
        text_input_tokens = self.llm_tokenizer(
            samples['question_str'],
            return_tensors="pt",
            padding="longest",
            truncation=True,
            max_length=32,
        ).to(fwd_results['mmt_seq_output'].device)

        from pythia.utils.objects_to_byte_tensor import dec_bytes2obj
        from collections import Counter
        gt_answers = []
        for ans in samples['gt_answers_enc']:
            answers=dec_bytes2obj(ans)
            count=Counter(answers)
            gt_answers.append(count.most_common(1)[0][0])

        # print(gt_answers)
        self.llm_tokenizer.truncation_side = 'right'
        text_output_tokens = self.llm_tokenizer(
            [t + self.llm_tokenizer.eos_token for t in gt_answers],
            return_tensors="pt",
            padding="longest",
            truncation=True,
            max_length=32,
        ).to(fwd_results['mmt_seq_output'].device)
        llm_tokens, input_part_targets_len = self.concat_text_input(
        ...
        empty_targets = (
            torch.ones(atts_llm.size(), dtype=torch.long).to(fwd_results['mmt_seq_output'].device).fill_(-100)
        )
        # do not apply loss to the additional image features
        # empty_add_targets = (
        #     torch.ones(atts_add_feature_llm.size(), dtype=torch.long).to(fwd_results.device).fill_(-100)
        # )
