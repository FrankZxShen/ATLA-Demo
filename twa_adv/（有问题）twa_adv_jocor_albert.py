...
from pytorch_transformers.modeling_bert import BertPreTrainedModel, BertEncoder, BertConfig
...
from pythia.utils.albert_utils import AlbertEmbeddings, AlbertTransformer, AlbertEncoder, AlbertLMPredictionHead, load_tf_weights_in_albert
...
class M4C(BaseModel):
    ...
    def build(self):
        ...
        self.cls = AlbertLMPredictionHead(
            self.text_bert.embeddings.word_embeddings.weight)
        self.Normal = nn.Linear(768, 128)
    ...
    def _build_char(self):
        ...
        if 'bert' in self.fb_featrue or 'char' in self.fb_featrue or 'bert' in self.config.ocr.features or 'char' in self.config.ocr.features:
            ...
            if 'char' in self.config.ocr.features:
                self.ocr_char_layernorm = AlbertLayerNorm(
                    self.mmt_config.hidden_size)
    ...
    def _build_txt_encoding(self):
        ...
        self.text_bert_config = AlbertConfig(**self.config.text_bert)
        if self.config.text_bert_init_from_bert_base:
            self.text_bert = TextBert.from_pretrained(
                'albert-base-v2', config=self.text_bert_config
            )
    ...
    def _build_obj_encoding(self):
        ...
        self.obj_feat_layer_norm = AlbertLayerNorm(self.mmt_config.hidden_size)
        self.obj_bbox_layer_norm = AlbertLayerNorm(self.mmt_config.hidden_size)
    ...
    def _build_ocr_encoding(self):
        ...
        if self.use_ocr_conf:
            ...
            self.ocr_conf_layer_norm = AlbertLayerNorm(
                self.mmt_config.hidden_size)
        ...
        self.ocr_feat_layer_norm = AlbertLayerNorm(self.mmt_config.hidden_size)
        self.ocr_bbox_layer_norm = AlbertLayerNorm(self.mmt_config.hidden_size)
    ...
    def _forward_pretrain_output(self, sample_list, fwd_results):
        ...
        # mmt_txt_output = self.Normal(mmt_txt_output)
...
## 给老子改（不要layout试一试）
class TextBert(AlbertPreTrainedModel):
...
# 对比 txt_inds=inputs_shape
    # token_type_ids=txt_type_mask
    def forward(self, txt_inds, txt_mask, adv, position_ids=None, txt_type_mask=None, head_mask=None, adv_delta_txt=None):
        token_type_ids = txt_type_mask
        ...
        #     txt_inds, token_type_ids=txt_type_mask)
        encoder_inputs = self.embeddings(
...
class MMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # self.Albert_embeddings = AlbertEmbeddings(config)
        self.encoder = BertEncoder(config)
        self.Normal = nn.Linear(config.hidden_size, 128)
        self.init_weights()
    ...
    def forward(self,
        ...
        # dec_emb = self.Albert_embeddings(dec_emb, prev_inds)
        ...
        assert not extended_attention_mask.requires_grad
        head_mask = [None] * self.config.num_hidden_layers
        # encoder_inputs = self.Normal(encoder_inputs)
        encoder_outputs = self.encoder(
            encoder_inputs,
            extended_attention_mask,
            head_mask=head_mask
...
class PrevPredEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        # self.nomal = nn.Linear(hidden_size, config.embedding_size)

        self.ans_layer_norm = AlbertLayerNorm(hidden_size, eps=ln_eps)
        self.ocr_layer_norm = AlbertLayerNorm(hidden_size, eps=ln_eps)
        self.emb_layer_norm = AlbertLayerNorm(hidden_size, eps=ln_eps)
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self):
        ...
        self.LayerNorm = AlbertLayerNorm(hidden_size, eps=1e-12)
