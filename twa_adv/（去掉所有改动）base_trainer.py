            ...
            for batch in self.train_loader
                with autocast(enabled=False)
                    ...
                    if batch['adv'] == True
                        ...
                        cmb_text = prepared_batch['cmb_text'].to(device=torch.device(cpu))
                        txt_embeds_init = torch.zeros([self.batch_size2, len(cmb_text[0]), 768])
                        ...
                        # 这个时候需要看看loss_pick1和loss_pick2长的是什么样子的    
                            ...
                            # self.writer.write(loss_pick1) #tensor(1259.3926, device='cuda0', grad_fn=AddBackward0)
                            # self.writer.write(==============loss_pick2==============)
                            # self.writer.write(loss_pick2) #tensor(1259.9485, device='cuda0', grad_fn=AddBackward0)
                            # exit()
                        # 直接获取klloss
                            # co_lambda = 0.1
                            ...
                            # self.writer.write(loss_pick.shape) #[816, 5200]
                            # self.writer.write(loss_pick.data.shape) #[816, 5200]
