            ...
            for batch in self.train_loader:
                with autocast(enabled=False):
                    ...
                    if batch['adv'] == True:
                        ...
                        ocr_fasttext_embeds_init = prepared_batch['context_feature_0'].to(device=torch.device("cpu"))
                        # ocr_fasttext = F.normalize(ocr_fasttext, dim=-1)
                        assert ocr_fasttext_embeds_init.size(-1) == 300

                        # 获取OCR PHOC 特征（604维）
                        ocr_phoc_embeds_init = prepared_batch['context_feature_1'].to(device=torch.device("cpu"))
                        # ocr_phoc = F.normalize(ocr_phoc, dim=-1)
                        assert ocr_phoc_embeds_init.size(-1) == 604

                        # 获取OCR Faster RCNN特征
                        ocr_fc6 = prepared_batch['image_feature_1'][:, :ocr_fasttext_embeds_init.size(1), :].to(device=torch.device("cpu"))
                        ...
                        if "textcls_scores" in list(model_output.keys()):
                            ...
                            # # mask = targets!=-1
                            # prepared_batch['gt_overlapcls_prob'] = F.softmax(gt_overlapcls_scores, dim=0)
                            # prepared_batch['gt_overlapcls_logprob'] = F.log_softmax(gt_overlapcls_scores, dim=0)
                        else:
                            gt_answer_scores = model_output['scores']
                            prepared_batch['gt_answer_prob'] = F.softmax(gt_answer_scores, dim=1)
                            prepared_batch['gt_answer_logprob'] = F.log_softmax(gt_answer_scores, dim=1)
                        # coteaching分支2
                        # gt_answer_scores2 = model2_output['scores']
                        # prepared_batch2['gt_answer_prob'] = F.softmax(gt_answer_scores2, dim=1)
                        # prepared_batch2['gt_answer_logprob2'] = F.log_softmax(gt_answer_scores2, dim=1)
                        # 以下过程在预训练，暂时没有分支
                        ...
                        # =====TARGET9: 更新损失======
                            ...
                            # loss_pick2 = self._extract_loss(report_adv2)
                        # ===================STEP5 计算jocor损失================
                        ...
                        # 直接获取klloss
                            # co_lambda = 0.1
                            # loss_pick = (loss_pick1 * (1-0.1) + loss_pick2 * (1-0.1) + \
                            #     0.1 * all_losses.loss_jocor_kl(model_adv_output["scores"], model2_adv_output["scores"], reduce=False) + \
                            #     0.1 * all_losses.loss_jocor_kl(model2_adv_output["scores"], model_adv_output["scores"], reduce=False))
                        ...
                        # =====TARGET12: 更新剪裁======
                            ...
                            txt_delta_step = (1e-5 * txt_delta_grad / denorm_txt).to(txt_delta)  # opts.adv_lr_txt
                            ...
                            img_delta_step = (1e-5 * img_delta_grad / denorm_img).to(img_delta)  # opts.adv_lr_txt
                            ...
                            ocr_delta_step = (1e-5 * ocr_delta_grad / denorm_ocr).to(ocr_delta)  # opts.adv_lr_txt
                            ...
                            ocr_fasttext_delta_step = (1e-5 * ocr_fasttext_delta_grad / denorm_ocr_fasttext).to(ocr_fasttext_delta)  # opts.adv_lr_txt
                            ...
                            ocr_phoc_delta_step = (1e-5 * ocr_phoc_delta_grad / denorm_ocr_phoc).to(ocr_phoc_delta)  # opts.adv_lr_txt
                        ...
                        # Arguments should be a dict at this point
                        model_output = self.model(prepared_batch)
                        model2_output = self.model2(prepared_batch)
                        report1 = Report(prepared_batch, model_output)
                        report2 = Report(prepared_batch, model2_output)
                        self.profile("Forward time")
                        self._update_meter(report1, self.meter)
                        self._update_meter(report2, self.meter)
                        loss_pick1 = self._extract_loss(report1)
                        loss_pick2 = self._extract_loss(report2)
                        loss_pick = (loss_pick1 * (1-0.1) + loss_pick2 * (1-0.1) + \
                                0.1 * all_losses.loss_jocor_kl(model_output["scores"], model2_output["scores"], reduce=False) + \
                                0.1 * all_losses.loss_jocor_kl(model2_output["scores"], model_output["scores"], reduce=False))
                            # self.writer.write("==============loss_pick&data==============")
                            # self.writer.write(loss_pick.shape) #[8/16, 5200]
                            # self.writer.write(loss_pick.data.shape) #[8/16, 5200]
                        ...
                        loss = torch.mean(loss_pick[:, ind_update])
                        self._backward(loss)
                        should_break = self._logistics(report1)
                        continue
