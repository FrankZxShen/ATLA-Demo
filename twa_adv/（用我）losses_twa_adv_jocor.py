...
class M4CDecodingBCEWithMaskLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        if sample_list['adv'] == True:
            ...
            loss = (loss + adv_kl_weight * kl_losses) / sample_list['adv_step']
    ...
    def forward(self, sample_list, model_output):
        if sample_list['adv'] == True:
            return (self.mlm_cls(sample_list, model_output) * 1. +
                    ...
                    self.overlap_cls(sample_list, model_output) * 1.) / sample_list['adv_step']
        else:
            return self.mlm_cls(sample_list, model_output) * 1. + \
                self.pollute_cls(sample_list, model_output) * 1. + \
...
class M4CDecodingPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        if sample_list['adv'] == True:
            ...
            kl_losses = F.kl_div(answer_logprob, sample_list['gt_textcls_prob'], reduce=False) + \
                F.kl_div(sample_list['gt_textcls_logprob'],
                         answer_prob, reduce=False)
            mean_value = torch.mean(kl_losses, dim=1, keepdim=True)  # 平均掉第二维
...
class M4CDecodingOverkapPretrainLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        # =====TARGET8: 计算KL Loss======
        if sample_list['adv'] == True:
            overlapcls_prob = F.softmax(scores, dim=0)
            overlapcls_logprob = F.log_softmax(scores, dim=0)
            epsilon = 1e-8  # small epsilon value to avoid zero
