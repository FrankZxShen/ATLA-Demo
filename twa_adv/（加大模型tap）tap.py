...
from transformers import BertConfig, BertPreTrainedModel, BertModel, LayoutLMModel
...
class M4C(BaseModel):
    def __init__(self, config):
        ...
        self.mmt_config = LayoutlmConfig(**self.config.mmt)
    ...
    def _build_txt_encoding(self):
        TEXT_BERT_HIDDEN_SIZE = 1024
        self.text_bert_config = LayoutlmConfig(**self.config.text_bert)
        if self.config.text_bert_init_from_bert_base:
            self.text_bert = TextBert.from_pretrained(
                'microsoft/layoutlm-large-uncased', config=self.text_bert_config
            )
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_feat = torch.cat(
            [ocr_fasttext, ocr_phoc, ocr_order_vectors],
            dim=-1
        )# [x,200,3152-2048]
        if self.con_mnt_rel:
        ...
        # print("1,",ocr_mmt_in.shape)# [x,200,1024]
        if 'char' in self.config.ocr.features:
...
class TextBert(LayoutLMModel):
    ...
    def forward(self, txt_inds, txt_mask, bbox, adv, txt_type_mask=None, adv_delta_txt=None):
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
                ...
                prev_inds,
                bbox=None):
        # Feature: TXT [32, 20, 1024]) OBJ [32, 100, 1024]) OCR[32, 50, 1024]) Answer torch.Size([5000, 1024]
